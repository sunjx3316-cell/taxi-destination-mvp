...
function destinationMarkup(d, id) {
  ...
  const buttons = [{ id: 'amap', label: '高德选路线', primary: true }, { id: 'baidu', label: '百度地图' }, { id: 'tencent', label: '腾讯地图' }, ...(!isNativeAndroid() ? [{ id: 'apple', label: '苹果地图' }] : [])];
...
async function openMap(provider, d) {
  ...
  const native = hasPoint ? { amap: `amapuri://route/plan/?sourceApplication=daonaer&dlat=${lat}&dlon=${lng}&dname=${name}&dev=0&t=0`, baidu: `baidumap://map/direction?destination=latlng:${lat},${lng}|name:${name}&mode=driving&coord_type=gcj02&src=daonaer`, tencent: `qqmap://map/routeplan?type=drive&tocoord=${lat},${lng}&to=${name}&referer=daonaer` } : {};
